...
server.modules_gcas_drug <- function(input, output, session) {
  ...
  plot_data <- eventReactive(input$search_bttn, {
    ...
    # 矩阵结构: 行 = 药物(rownames), 列 = 数据集(colnames)
    # R 的 as.numeric(mat) 按列展开(column-major): 药物交替、数据集成块
    # 变量名与最终列名不一致，由下方 data.frame() + colnames() 重命名决定:
    #   data.frame 第1列(Drug 变量) -> 重命名为 "Dataset"
    #   data.frame 第2列(Dataset 变量) -> 重命名为 "Drug"
    # 因此 Drug 变量装数据集名(成块), Dataset 变量装药物名(交替)

    # 获取矩阵维度
    n_drugs <- nrow(p$r)
    n_datasets <- ncol(p$r)
    total_rows <- n_datasets * n_drugs

    # 创建两列（与 as.numeric 按列展开顺序对齐）
    Dataset <- rep(rownames(p$r), n_datasets)      # 药物名交替 (最终列名 Drug)
    Drug <- rep(colnames(p$r), each = n_drugs)      # 数据集名成块 (最终列名 Dataset)
    ...
    # 重命名列以便显示
    colnames(result_df) <- c( "Dataset", "Drug","n", "r", "t", "p", "p.adj", "ci_lower", "ci_upper", "Significance")
  ...
  observeEvent(eventExpr = input$tbl_rows_selected,{
    ...
    if (length(s)) {
      ...
      output$scatter_plot <- renderPlot(width = 450,
                                        height = 300,{
                                          # 使用新的返回格式
                                          cor_result <- drug_corr_func()
                                          viz_corplot(cor_result, colnames(cor_result)[5], colnames(cor_result)[6],
                                                     method = input$cor_method, x_lab = " expression", y_lab = " sensitive score")
